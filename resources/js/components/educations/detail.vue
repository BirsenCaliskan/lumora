  ...
  <template v-if="education && Object.keys(education).length">
    ...
    <div class="max-w-screen-xl xl:mx-auto flex max-lg:flex-col gap-6 mb-14 3xs:mb-7">
      <div class="grow min-w-0 space-y-4 h-fit">
        ...
        <div v-if="education.detail_list && Object.keys(education.detail_list).length" class="space-y-6 mx-4">
          <div class="max-w-sm mx-auto bg-gradient-to-l from-white via-red-301 to-black w-full py-6 rounded-lg text-xl leading-5 3xs:max-lg:text-base font-bold">
            {{ education.name }} Hakkında
          </div>
          <div class="">
            <div v-for="data in education.detail_list" :key="`accordion-${data.id}`" class="mb-3">
              <button type="button" @click="accordionButton(data.id)" aria-label="button" class="bg-gray-709 w-full rounded text-left flex items-center justify-between gap-1 px-4 py-2">
                <span>{{ data.name }}</span>
                <svg width="33" height="21" viewBox="0 0 33 21" fill="none" xmlns="http://www.w3.org/2000/svg" class="size-5 [&.active]:rotate-180">
                  <path fill-rule="evenodd" clip-rule="evenodd" d="M18.9289 19.8043C18.2843 20.448 17.4106 20.8096 16.4997 20.8096C15.5888 20.8096 14.7151 20.448 14.0705 19.8043L1.10428 6.8426C0.785126 6.52329 0.53199 6.14425 0.359325 5.72712C0.186657 5.30999 0.0978394 4.86293 0.0979462 4.41147C0.098053 3.96002 0.18708 3.513 0.359947 3.09595C0.53281 2.6789 0.786121 2.29998 1.10542 1.98083C1.42473 1.66167 1.80377 1.40854 2.2209 1.23587C2.63803 1.0632 3.08509 0.974384 3.53655 0.974491C3.98801 0.974598 4.43502 1.06363 4.85207 1.23649C5.26912 1.40935 5.64804 1.66267 5.9672 1.98197L16.4997 12.5122L27.0322 1.97968C27.3491 1.65122 27.7283 1.38916 28.1476 1.20881C28.5669 1.02846 29.018 0.933428 29.4744 0.929247C29.9308 0.925068 30.3835 1.01183 30.806 1.18447C31.2286 1.35711 31.6125 1.61217 31.9354 1.93478C32.2583 2.25738 32.5137 2.64106 32.6868 3.06343C32.8598 3.48581 32.947 3.93841 32.9432 4.39484C32.9395 4.85127 32.8449 5.30238 32.6649 5.72185C32.485 6.14133 32.2233 6.52076 31.8951 6.83801L18.9289 19.8043Z" fill="white"/>
                </svg>
              </button>
              <div :class="{'active': accordion === data.id}" class="bg-gradient-to-b from-black via-red-800/60 to-black rounded-lg [&.active]:p-4 sm:[&.active]:p-6 lg:[&.active]:p-8 border border-white h-0 opacity-0 overflow-hidden invisible scrollbar [&.active]:h-[300px] [&.active]:overflow-y-auto [&.active]:opacity-100 [&.active]:mt-2.5 [&.active]:visible duration-300" v-html="data.detail"></div>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- educations -->
    <div class="max-w-screen-2xl 2xl:mx-auto mx-4 space-y-6 mb-14 3xs:mb-7">
      <!-- title -->
      ...
      <div class="w-full overflow-y-hidden overflow-x-auto scrollbar touch-auto">
        <div class="grid grid-cols-6 max-sm:w-[700px]">
          <template v-for="data in newEducations" :key="`new-${data.id}`">
            <router-link :to="`/egitim/${data.url}`" :title="data.education_name" class="block relative first:ml-0 -ml-8 hover:z-10 duration-200">
              <img :src="`${BASEURL}${data.picture}`" width="391" height="435" alt="" loading="lazy" fetchpriority="low" decoding="async" class="w-full h-full rounded-3xl border-[3px] border-red-301 group-[.swiper-slide-active]:opacity-80 group-[.swiper-slide-active]:border group-[.swiper-slide-active]:border-white">
              <div class="absolute inset-0 size-full bg-gradient-to-t from-black/60 from-10% rounded-b-3xl"></div>
              <div class="absolute bottom-2 inset-x-0 text-3xl 3xs:max-lg:text-xl leading-[1.3] text-white font-bold text-center px-3 line-clamp-3" v-html="data.name"></div>
              <div class="absolute inset-0 rounded-3xl bg-black/40 3xs:max-lg:hidden opacity-0 group-hover:opacity-100 duration-300 flex items-center justify-center">
                <div class="">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" class="size-8">
                    <g fill="none" stroke="currentColor" stroke-width="1">
                      <circle cx="8" cy="8" r="2.5"/>
                      <path stroke-linecap="round" d="M15.42 8.29c.12-.18.12-.4 0-.58c-.7-1-3.28-4.21-7.42-4.21S1.28 6.71.58 7.71c-.12.18-.12.4 0 .58c.7 1 3.28 4.21 7.42 4.21s6.72-3.21 7.42-4.21z"/>
                    </g>
                  </svg>
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    async callModal(){
      await openModal('call_form',{education_id: this.education.id,emit: () => {
        }
      });
    },
    accordionButton(id){
